...
public class SwearVengeance : Combo {
    public override void Play(GameState gameState) {
        ...
        if (selectedEnemy.Health <= selectedEnemy.MaxHealth / 5) { selectedEnemy.Health = 0; }
        if (gameState.Player.Health <= gameState.Player.MaxHealth / 5) { gameState.SpectacleBuffer += 50; }
    }
}
